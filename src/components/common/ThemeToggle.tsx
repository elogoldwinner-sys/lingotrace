 ...
 * the kid-mode homepage; `variant="header"` is the same look at the smaller
 * size used in the kid-mode dashboard top bar; the default variant is used
 * everywhere else.
 */
export default function ThemeToggle({
  ...
  const look =
    variant === "header"
      ? "gap-2 rounded-full border-2 border-gold-300 bg-white px-4 py-2 text-sm font-extrabold text-navy-700 shadow-[0_6px_14px_-8px_rgba(255,152,0,0.7)] ease-bouncy hover:-translate-y-0.5"
      : variant === "pill"
  ...
  const iconSize = variant === "pill" ? 18 : variant === "header" ? 17 : 14;
